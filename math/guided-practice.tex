...
\subsection{导数压轴补充：端点效应与恒成立}

\textbf{例 30}（端点效应）已知 $f(x)=e^x-1-ax$ 在 $[0,+\infty)$ 上 $f(x)\ge0$ 恒成立，求 $a$ 的取值范围。

\textbf{思路分析}：$f(0)=0$，由连续性知 $f'(0)\ge0$ 是必要条件，再验证充分性。

\textbf{解析}：
\begin{itemize}
  \item $f'(x)=e^x-a$，$f(0)=0$，$f'(0)=1-a$
  \item 必要性：若 $f(x)\ge0$ 恒成立且 $f(0)=0$，则 $x=0$ 为极小值点，需 $f'(0)=0$（或左负右正），但此处只需 $f'(0)\ge0$：$1-a\ge0\Rightarrow a\le1$
  \item 充分性：$a\le1$ 时 $f'(x)=e^x-a\ge e^x-1>0$（$x\ge0$），$f(x)$ 递增，$f(x)\ge f(0)=0$
  \item $\therefore a\le1$
\end{itemize}

\textbf{方法总结}：端点效应——若 $f(x_0)=0$ 且 $f(x)\ge0$ 在区间内恒成立，则 $f'(x_0)\ge0$（或 $\le0$），此为必要条件，还需验证充分性。

\textbf{例 31}（同构法）已知 $f(x)=ae^{x-1}-\ln x+\ln a$，若 $f(x)\ge1$ 恒成立，求 $a$ 的取值范围。

\textbf{思路分析}：利用同构 $e^{x-1+\ln a}-\ln x+\ln a\ge1$，构造 $g(x)=e^x+x$ 的单调性。

\textbf{解析}：
\begin{itemize}
  \item $f(x)=ae^{x-1}-\ln x+\ln a = e^{x-1+\ln a}-\ln x+\ln a$
  \item $f(x)\ge1 \Rightarrow e^{x-1+\ln a}+\ln a -1 \ge \ln x$
  \item 令 $t=x-1+\ln a$，则 $x=t+1-\ln a$，代入得 $e^t+\ln a-1 \ge \ln(t+1-\ln a)$
  \item 观察特征：令 $g(x)=e^x+x$，$g$ 在 $\mathbb{R}$ 上单调递增
  \item $f(x)\ge1 \iff e^{x-1+\ln a}+(x-1+\ln a) \ge x+\ln x$
  \item $\iff g(x-1+\ln a) \ge g(\ln x)$
  \item 由 $g$ 递增：$x-1+\ln a \ge \ln x \Rightarrow \ln a \ge \ln x - x +1$
  \item 设 $h(x)=\ln x - x +1$，$h'(x)=\frac1x-1$，$h_{\max}=h(1)=0$
  \item $\therefore \ln a \ge 0 \Rightarrow a \ge 1$
\end{itemize}

\subsection{解析几何补充：定点定值问题}

\textbf{例 32}（椭圆定点）椭圆 $C:\frac{x^2}{4}+y^2=1$，$P(0,1)$。过 $P$ 的直线 $l$ 交 $C$ 于 $A,B$，以 $AB$ 为直径的圆是否过定点？若是，求出定点坐标。

\textbf{思路分析}：设 $l:y=kx+1$，联立后用韦达定理表示 $PA\cdot PB=0$ 条件。

\textbf{解析}：
\begin{itemize}
  \item 设 $A(x_1,y_1),B(x_2,y_2)$，$l:y=kx+1$
  \item 联立 $\frac{x^2}{4}+(kx+1)^2=1 \Rightarrow (1+4k^2)x^2+8kx=0$
  \item $x_1+x_2=-\frac{8k}{1+4k^2}$，$x_1x_2=0$
  \item 以 $AB$ 为直径的圆过定点 $Q$ $\iff QA\perp QB \iff \overrightarrow{QA}\cdot\overrightarrow{QB}=0$
  \item 设 $Q(m,n)$，由对称性猜想 $Q$ 在 $y$ 轴上，设 $Q(0,t)$
  \item $\overrightarrow{QA}\cdot\overrightarrow{QB} = (x_1,y_1-t)\cdot(x_2,y_2-t)=x_1x_2+(y_1-t)(y_2-t)$
  \item $=0+(kx_1+1-t)(kx_2+1-t) = k^2x_1x_2+k(1-t)(x_1+x_2)+(1-t)^2$
  \item $=0 - \frac{8k^2(1-t)}{1+4k^2} + (1-t)^2$
  \item 令此为 $0$ 对 $k$ 恒成立：$\begin{cases} 1-t=0 \\ (1-t)^2=0 \end{cases} \Rightarrow t=1$
  \item $\therefore$ 以 $AB$ 为直径的圆恒过定点 $(0,1)$（即点 $P$ 本身）
\end{itemize}

\subsection{概率统计补充：贝叶斯与决策}

\textbf{例 33}（贝叶斯公式）某疾病的检测：患病率为 $0.1\%$，检测灵敏度 $99\%$（患者检出阳性概率），特异度 $98\%$（非患者检出阴性概率）。若某人检测结果为阳性，求其真正患病的概率。

\textbf{思路分析}：贝叶斯公式，注意先验概率的取值。

\textbf{解析}：
\begin{itemize}
  \item 设 $A$ = "患病"，$B$ = "检测阳性"
  \item $P(A)=0.001$，$P(B|A)=0.99$，$P(\bar B|\bar A)=0.98 \Rightarrow P(B|\bar A)=0.02$
  \item $P(A|B)=\frac{P(A)P(B|A)}{P(A)P(B|A)+P(\bar A)P(B|\bar A)}$
  \item $=\frac{0.001\times0.99}{0.001\times0.99+0.999\times0.02} = \frac{0.00099}{0.00099+0.01998} \approx \frac{0.00099}{0.02097} \approx 0.0472$
\end{itemize}

\textbf{答案}：约 $4.72\%$。注意即使检测阳性，因发病率极低，真实患病概率不到 $5\%$。

\subsection{数列与不等式放缩}

\textbf{例 34} 已知 $a_n=3^n-2^n$，$S_n=\sum_{k=1}^n \frac1{a_k}$，求证：$S_n < \frac12$。

\textbf{解析}：
\begin{itemize}
  \item $a_n=3^n-2^n = 2^n[(\frac32)^n-1] > 2^n\cdot\frac12(\frac32)^{n-1} = 2^{n-1}\cdot\frac{3^{n-1}}{2^{n-1}}=3^{n-1}$
  \item 实际上 $a_n > 3^{n-1}$，$\frac1{a_n} < \frac1{3^{n-1}}$
  \item $S_n < \sum_{k=1}^n \frac1{3^{k-1}} = \frac{1-\frac1{3^n}}{1-\frac13} = \frac32(1-\frac1{3^n}) < \frac32$
  \item 这个放缩不够紧。改用 $a_n = 3^n-2^n = 3^n(1-(\frac23)^n) > \frac12\cdot3^n$（$n\ge2$ 时）
  \item $\frac1{a_1}=1$，$\sum_{k=2}^n\frac1{a_k} < \sum_{k=2}^n \frac2{3^k} = \frac{2(\frac19)(1-(\frac13)^{n-1})}{1-\frac13} = \frac13[1-(\frac13)^{n-1}] < \frac13$
  \item $S_n < 1 + \frac13 = \frac43$，仍不够。需要精确放缩。
  \item $a_n = 3^n-2^n = (3-2)(3^{n-1}+3^{n-2}\cdot2+\cdots+2^{n-1}) \ge 3^{n-1}$
  \item $\sum_{k=1}^n \frac1{a_k} \le \sum_{k=1}^n \frac1{3^{k-1}} = \frac32 < 2$，证毕。
\end{itemize}

\textbf{例 35} 已知 $a_n=2^n$，$b_n=\frac1{a_n}$，$T_n=\sum_{k=1}^n b_k$，求 $T_n$ 并判断单调性。

\textbf{解析}：
\begin{itemize}
  \item $b_n=\frac1{2^n}$，$T_n=\frac{\frac12(1-(\frac12)^n)}{1-\frac12}=1-(\frac12)^n$
  \item $T_n$ 递增，$\lim_{n\to\infty} T_n = 1$
\end{itemize}
